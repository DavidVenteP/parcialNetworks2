...
async function ingresarPaquete(producto) {
    ...
    try {
        ...
        resultado = await conexion.execute('INSERT INTO notification VALUES(null,?,?,?,?)',
            [resultado[0].insertId, json1.user_id, json1.status_id, new Date(new Date().toLocale("es-CO", {timeZone: 'America/Bogota'}))]);
        // Cierra la conexión
        await conexion.end();
        return true
    } catch (error) {
...
async function updatePackage(producto) {
    ...
    try {
        ...
        // * Crea registro en NOTIFICATION con el registro creado en PAQUETE
        resultado = await conexion.execute('INSERT INTO notification VALUES(null,?,?,?,?)',
            [json1.id, json1.user_id, json1.status_id, new Date(new Date().toLocale("es-CO", {timeZone: 'America/Bogota'}))]);
